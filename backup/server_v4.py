...
def process_instruction(instruction):
    ...
    # --- MODIFIED: Safe checks for None images to prevent OpenCV crash ---
    if drone_1.current_image is None or drone_2.current_image is None:
        print("Video streams not ready. current_image is None. Ignoring instruction.")
        return
    # ---------------------------------------------------------------------
    ...
    # --- MODIFIED: Inject comprehensive initial status (My Status + Teammate Status + Yaw) ---
    ...
    while True:
        # --- v4 MODIFIED: Parallel LLM Request Implementation Using Multi-Threading ---
        res_drone1 = [None, None]  # Format: [output_text, messages]
        res_drone2 = [None, None]

        def worker_drone1():
            if not is_drone1_stop:
                out, msgs = ask(drone1_messages)
                res_drone1[0] = out
                res_drone1[1] = msgs

        def worker_drone2():
            if not is_drone2_stop:
                out, msgs = ask(drone2_messages)
                res_drone2[0] = out
                res_drone2[1] = msgs

        # v4 Start both API requests at the exact same time
        t1 = threading.Thread(target=worker_drone1)
        t2 = threading.Thread(target=worker_drone2)
        t1.start()
        t2.start()
        t1.join()
        t2.join()
        ...
        # --- MODIFIED: Process Drone 1 results sequentially after parallel block ---
        if not is_drone1_stop and res_drone1[0] is not None:
            output_text_drone1, drone1_messages = res_drone1[0], res_drone1[1]
        ...
        # --- MODIFIED: Process Drone 2 results sequentially after parallel block ---
        if not is_drone2_stop and res_drone2[0] is not None:
            output_text_drone2, drone2_messages = res_drone2[0], res_drone2[1]
        ...
        # --- MODIFIED: Safe check before grabbing new frames in loop ---
        if drone_1.current_image is None or drone_2.current_image is None:
            print("Video stream lost during loop. Breaking to prevent crash.")
            break
        # ---------------------------------------------------------------
        ...
        # --- MODIFIED: Inject comprehensive loop status (My Status + Teammate Status + Yaw) ---
        state_1 = drone_states[1]
        state_2 = drone_states[2]
